...
//}
const universities = graduates.map(cur => {
  return cur.university;
});
universities.sort();
...
const contactInfo = graduates.map(cur => {
  return `${cur.first_name} ${cur.email}`;
});
console.log(contactInfo);

/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called unisWithUni that contains them all. This will be an array of objects. Log the result. */
const unisWithUni = graduates.filter(cur => {
  return cur.university.includes('Uni');
});
...
const lowCaseAnimalNames = zooAnimals.map(cur => {
...
*/
const lowPopulationAnimals = zooAnimals.filter(cur => {
